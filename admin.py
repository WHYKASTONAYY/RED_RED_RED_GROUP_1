...
async def handle_adm_set_media(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    if not is_primary_admin(query.from_user.id): return await query.answer("Access denied.", show_alert=True)
    lang, lang_data = _get_lang_data(context) # Use helper
    set_media_prompt_text = lang_data.get("set_media_prompt_plain", "Send a photo, video, GIF, or document to display above all messages:")
    ...
    await query.answer("Send photo, video, GIF, or document.")


async def handle_adm_bot_media_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles the admin sending bot media (photo, video, or GIF)."""
    if not update.message or not update.effective_user:
        return
    ...
    if context.user_data.get("state") != "awaiting_bot_media":
        logger.debug(f"Ignoring bot media message from user {user_id}, state is not 'awaiting_bot_media'")
        return
    ...
    if update.message.photo:
        ...
        # Support for documents (PDFs, text files, etc.)
        media_type = "document"
        file_id = update.message.document.file_id
    else:
        await send_message_with_retry(context.bot, chat_id, 
            "❌ Please send a photo, video, GIF, or document. Other file types are not supported.", 
            parse_mode=None)
        return
    ...
    try:
        ...
        if media_type == "photo":
            ...
            # Use the original file extension from the document
            original_filename = update.message.document.file_name
            if original_filename and '.' in original_filename:
                file_extension = '.' + original_filename.split('.')[-1].lower()
            else:
                file_extension = ".pdf"  # Default for documents
        else:
            file_extension = ".dat"
        
        # Create filename with timestamp to avoid conflicts
        timestamp = int(time.time())
        filename = f"bot_media_{timestamp}{file_extension}"
        ...
        # Download the file
        await file_obj.download_to_drive(custom_path=media_path)
        
        # Verify the file was downloaded successfully
        if not os.path.exists(media_path) or os.path.getsize(media_path) == 0:
            raise IOError(f"Downloaded file {media_path} is missing or empty.")
        
        # Update global BOT_MEDIA variable
        global BOT_MEDIA
        BOT_MEDIA = {
            "type": media_type,
            "path": media_path
        ...
        logger.info(f"Bot media updated by admin {user_id}: type={media_type}, path={media_path}")
        
    except telegram_error.TelegramError as e:
        logger.error(f"Telegram error downloading bot media: {e}")
        await send_message_with_retry(context.bot, chat_id, 
            f"❌ Error downloading media from Telegram: {str(e)}", parse_mode=None)
    except IOError as e:
        logger.error(f"IO error saving bot media: {e}")
        await send_message_with_retry(context.bot, chat_id, 
            f"❌ Error saving media file: {str(e)}", parse_mode=None)
    except Exception as e:
        logger.error(f"Unexpected error processing bot media: {e}", exc_info=True)
        await send_message_with_retry(context.bot, chat_id, 
            f"❌ Unexpected error: {str(e)}", parse_mode=None)
